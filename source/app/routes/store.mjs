...
import { deleteInventory } from './inventory.mjs';
...
export const createStore = async function(storeId, request) {
    try {
...
export const deleteStore = async function(storeId) {
    try {
...
export const getStoreById = async function(storeId, query) {
    try {
...
export const getStores = async function(query) {
    try {
        const float = ['latitude', 'longitude', 'zip_code'];
...
export const updateStore = async function(storeId, request) {
    try {
...
router.delete('/:storeId', async function(req, res) {
    ...
    const deleted = await Promise.all([
        deleteInventory(undefined, storeId),
        deleteStore(storeId)
    ]);
    
    status = !Fn.containsString(deleted, false) ? 200 : 500;
    response = Fn.setResponse(status);
    res.status(status).json(response);
    Fn.httpLog(req, status, JSON.stringify(response).length);
});
